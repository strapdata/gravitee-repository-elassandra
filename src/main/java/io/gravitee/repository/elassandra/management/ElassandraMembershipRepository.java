...
public class ElassandraMembershipRepository extends ElassandraCrud<Membership, String> implements MembershipRepository {
    ...
    public void deleteMembers(MembershipReferenceType referenceType, String referenceId) throws TechnicalException {
        LOGGER.debug("Delete Members [{}]-[{}]", referenceType, referenceId);

        if (referenceType == null || Strings.isNullOrEmpty(referenceId)) {
            LOGGER.warn("Delete Members requires two parameter, received : [{}]-[{}]", referenceType, referenceId);
            throw new IllegalStateException("deleteMembers require two parameter values");
        } else {
            ...
            for (Row row : resultSet) {
                Membership membership = fromRow(row);
                LOGGER.debug("Delete Member '{}'", membership);
                delete(membership);
            }
        }
    }
